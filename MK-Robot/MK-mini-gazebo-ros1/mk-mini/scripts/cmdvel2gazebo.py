...
class CmdVel2Gazebo:
    def __init__(self):
        ...
        # 发布器初始化
        self.pub_steerL = rospy.Publisher('/mk_mini/front_left_steering_position_controller/command', Float64, queue_size=1)
        self.pub_steerR = rospy.Publisher('/mk_mini/front_right_steering_position_controller/command', Float64, queue_size=1)
        self.pub_rearL = rospy.Publisher('/mk_mini/rear_left_velocity_controller/command', Float64, queue_size=1)
        self.pub_rearR = rospy.Publisher('/mk_mini/rear_right_velocity_controller/command', Float64, queue_size=1)

        # 车辆参数
        self.L = 0.6            # 轴距 (m)
        self.T_front = 0.517    # 前轮轮距 (m)
        self.T_rear = 0.517     # 后轮轮距 (m)
